...
const BlogTabs = (props) => {
  ...
  let handleTabChange = (key, data = []) => {
    ...
    setActiveTab(key)
    if (data.length === 0) {
      filteredData = blogsData.results.filter(item => item.category.name.toLowerCase().trim() === key.toLowerCase().trim());
    }
    else {
      filteredData = data.results.filter(item => item.category.name.toLowerCase().trim() === key.toLowerCase().trim());
  ...
  return (
    <>
      <Tabs 
        defaultActiveKey={tabsData && tabsData.length > 0 ? tabsData[0].name.toLowerCase() : "news"}
      ...
      >
        ...
        {tabsData && tabsData.map((tab) => (
          <Tab key={tab.id} eventKey={tab.name.toLowerCase()} title={capitalizeFirstLetter(tab.name)}>
            <Row>
              <Row className="justify-content-center mb-4">
                <Col md={1}>
                  <Button variant="primary" onClick={() => handlePrev(blogsData.previous)} disabled={prevValue === null}>
                    &lt; Prev
                  </Button>
                </Col>
                <Col md={10}>
                ...
                <Col md={1}>
                  <Button variant="primary" onClick={() => handleNext(blogsData.next)} disabled={nextValue === null}>
                    Next &gt;
                  </Button>
                </Col>
              </Row>
              {blogsTempData.results && blogsTempData.results.map((data) =>
              (
